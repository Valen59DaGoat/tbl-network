...
async function renderStandings(){
  ...
  el.innerHTML = teams.map(t => `
    <tr>
      <td class="team-name">${escapeHtml(t.name)} <span style="opacity:.55">(${escapeHtml(t.abbr)})</span></td>
...
async function renderSchedule(){
  ...
  el.innerHTML = data.map(g => `
    <tr>
      ...
      <td>${escapeHtml(g.away_team)} @ ${escapeHtml(g.home_team)}</td>
...
async function renderTeams(){
  ...
  el.innerHTML = teams.map(t => `
    <div class="card">
      <h3>${escapeHtml(t.name)} <span style="opacity:.5">${escapeHtml(t.abbr)}</span></h3>
...
async function renderRankings(){
  ...
  el.innerHTML = data.map(r => `
    <div class="card">
      <h3><span class="rank-badge">#${r.rank}</span>${escapeHtml(r.team)}</h3>
